	...
	extends \threewp_broadcast\premium_pack\base
{
	...
	**/
	public function broadcast_acf_add_field( $action )
	{
		...
		$this->debug( 'Attempting to add a %s field with the value %s', $field->type, $field->value );
		try
		{
			switch( $field->type )
			{
	...
	**/
	public function broadcast_acf_parse_field( $action )
	{
		...
		$this->debug( 'Field is of type: %s', $action->field->type );
		switch( $action->field->type )
		{
			...
			case 'repeater':
				...
				$this->debug( 'Repeater field %s found with %s subfields.', $action->field->name, count( $action->field->sub_fields ) );
	...
	public function add_gallery_field( $action )
	{
		...
		foreach( $field->value as $value )
		{
			try
			{
				...
				$this->debug( 'Adding attachment data for the image %s.', $value );
	...
	**/
	public function add_image_file_field( $action )
	{
		...
		foreach( $field->value as $image_id )
		{
			if ( ! $image_id )
				continue;
	...
	**/
	public function save_post_fields( $bcd, $target_id = null )
	{
		...
		foreach( $fields as $field_index => $field )
		{
			...
			$this->debug( 'Parsing field: %s (%s)', $field[ 'label' ], $field[ 'key' ] );
	...
	**/
	public function save_taxonomy_fields( $bcd )
	{
		...
		foreach( $bcd->parent_post_taxonomies as $taxonomy => $terms )
		{
			...
			$this->debug( 'Taxonomy %s has the following field groups: %s', $taxonomy, $field_groups );
