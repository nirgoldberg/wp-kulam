	...
	extends \threewp_broadcast\premium_pack\base
{
	...
	**/
	public function threewp_broadcast_ubs_edit_modification_settings( $action )
	{
		...
		$display_broadcast_menu = $fs->checkbox( 'display_broadcast_menu' )
			...
			// Input title
			->description( __( 'Display the broadcast menu in the post overview showing link information.', 'threewp_broadcast' ) )
			// Input label
		...
		$display_broadcast_meta_box = $fs->checkbox( 'display_broadcast_meta_box' )
			...
			// Input title
			->description( __( 'Display the broadcast meta box in the post overview showing link information.', 'threewp_broadcast' ) )
			// Input label
	...
	public function cached_user_modifications()
	{
		...
		foreach( $modifications as $modification_id => $modification )
		{
			...
			foreach( $modification as $criterion )
			{
				...
				if ( $applicable )
					$this->debug( 'Criteria %s is applicable', $criterion->id );
				switch( $criterion->get_operator() )
				{
					case 'and':
						if ( $use_modification === null )
							$use_modification = true;
						$use_modification &= $applicable;
					break;
					case 'or':
						if ( $use_modification === null )
	...
	**/
	...
	/**
		...
		@since		20131016
	**/
	public function get_modifications( $modification_ids = [] )
	{
