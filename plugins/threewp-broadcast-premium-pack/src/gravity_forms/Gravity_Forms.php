	...
	extends \threewp_broadcast\premium_pack\classes\Shortcode_Preparser
{
	...
	**/
	...
	public function copy_item( $bcd, $item )
	{
		...
		$query = sprintf( "SELECT * FROM `%s` WHERE `title` = '%s'", $table, $form->title );
		$result = $wpdb->get_row( $query );

		if ( count( $result ) < 1 )
		{
			$columns = '`title`, `date_created`, `is_active`, `is_trash`';
			$query = sprintf( "INSERT INTO `%s` ( %s ) ( SELECT %s FROM `%s` WHERE `title` ='%s' )",
		...
		$query = sprintf( "UPDATE `%s` SET `is_active` = '%d', `is_trash` = '%d' WHERE `id` = '%s'",
			$this->rg_gf_table( 'form', $target_prefix ),
			$form->is_active,
			$form->is_trash,
			$new_form_id
