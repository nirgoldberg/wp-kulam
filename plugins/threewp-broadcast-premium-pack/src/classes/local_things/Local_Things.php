	...
	extends \threewp_broadcast\premium_pack\base
{
	...
	**/
	...
	public function threewp_broadcast_parse_content( $action )
	{
		...
		if ( ! isset( $bcd->$key ) )
			return $this->debug( 'No object.' );

		if ( ! $bcd->$key->has( $action->id ) )
			return $this->debug( 'No local things for content %s', $action->id );
	...
	**/
	public function threewp_broadcast_preparse_content( $action )
	{
		...
		if ( ! is_object( $item ) )
			return $this->debug( 'Meta box item not found.' );

		$checkbox = $item->inputs->get( $key );
		if ( ! $checkbox->is_checked() )
			return $this->debug( 'Checkbox not checked.' );

		// Get the post content
		$content = $action->content;

		if ( strlen( $content ) < 1 )
			return $this->debug( 'Content is empty.' );
		...
		foreach( $this->get_find_elements() as $element_tag => $element_attribute )
		{
			...
			$this->debug( '%s anchors found.', count( $elements ) );
		...
		if ( count( $things ) < 1 )
			return $this->debug( 'No things found.' );
