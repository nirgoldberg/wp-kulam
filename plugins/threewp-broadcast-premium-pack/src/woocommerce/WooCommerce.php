...
extends \threewp_broadcast\premium_pack\base
{
	...
	**/
	public function settings()
	{
		...
		$input_sync_orders = $fs->checkbox( 'sync_orders' )
			...
			// Input title
			->description( __( 'Expermimental as of 2016-07-14: Automatically broadcast orders between shared products? The Back To Parent Broadcast add-on must be enabled for this to work.', 'threewp_broadcast' ) )
			// Input label
	...
	**/
	public function update_order( $order_id )
	{
		...
		if ( ! $this->back_to_parent() )
			return $this->debug( 'The Back To Parent add-on must be enabled in order to sync orders.' );
		...
		if ( $order_bcd->has_linked_children() OR $order_bcd->get_linked_parent() !== false )
		{
	...
	**/
	public function save_variations( $bcd )
	{
		...
		$variations = $bcd->woocommerce->product->get_available_variations();
