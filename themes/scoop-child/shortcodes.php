...
function kulam_generate_homepage_tiles( $atts ) {
    ...
    foreach ($homepage_tiles as $homepage_tile) {
        ...
        $markup .= '<div class="tile-box-wrapper col-md-3">' .
            '<a href="' . $category_link .'"  role="link" class="tile-box-link">' .
            '<div class="tile-box">' .
                '<div class="tile-box-content">' .
                '<h2>' . $category_title . '</h2>' .
                '<div class="tile-text show-on-hover">' . $category_description . '</div>' .
               // '<span class="view-all">' . $category_count . ' ' . $a['view-all_label'] . '</span>' .
                '</div>' .
            '</div>' .
            '</a>' .
        '</div>';
